...
namespace OnlyMyGame.Runtime
{
    ...
    public sealed class GameController : MonoBehaviour
    {
        ...
        private readonly RuleVm vm = new RuleVm();
        ...
        private void Start()
        {
            // 씬에 배치된 UI 위젯에 이벤트 연결
            if (endTurnButton != null) endTurnButton.onClick.AddListener(EndTurn);
            if (blockPanel != null)
            {
                var retry = blockPanel.transform.Find("Retry")?.GetComponent<Button>();
                if (retry != null) retry.onClick.AddListener(RetryRules);
                var quit = blockPanel.transform.Find("Quit")?.GetComponent<Button>();
                if (quit != null) quit.onClick.AddListener(SaveAndQuit);
            }
            RefreshHud();
        }
        ...
        {
            try { var saved = ReadSave(SaveKey) ?? ReadSave(BackupKey); game = saved ?? WorldGenerator.Create(Environment.TickCount); }
            catch { game = WorldGenerator.Create(Environment.TickCount); }
            BuildWorld();
        ...
        private void SpawnUnitVisual(UnitState unit)
        {
            ...
            // KayKit 캐릭터 프리셋은 이미 0.5 스케일로 저장되어 있으므로, 프리셋이 있으면 1.0 기준으로 배치
            marker.transform.localScale = prefab == null ? Vector3.one * .42f : Vector3.one * .9f;
            ...
            text.text = unit.factionId == 1 ? "★" : unit.factionId == 2 ? "☠" : "¤";
        ...
        private void SpawnBuildingVisual(BuildingState building)
        {
            ...
            landmark.transform.localScale = prefab == null ? Vector3.one * (.38f + building.level * .04f) : Vector3.one * (.8f + building.level * .06f);
        ...
        private void RenderVisibility()
        {
            ...
            foreach (var tile in game.map)
            {
                if (tileVisuals.TryGetValue(tile.position, out var go)) go.SetActive(tile.explored);
            }
            foreach (var building in game.buildings)
            {
                if (buildingVisuals.TryGetValue(building.id, out var go))
                {
                    ...
                    go.SetActive(tile != null && tile.visible);
                }
            }
        ...
        private void ShowUnitBubbles(UnitState unit)
        {
            ...
            var usedSp = commands.Sum(c => Cost(c.type));

            if (unit.factionId == 1)
            {
                if (usedSp + Cost(CommandType.Move) <= player.sp)
                    items.Add(MakeBubble("이동", new Color(.3f, .8f, 1f), "인접 타일로 이동 (SP 1)", () => Queue(CommandType.Move)));
                if (usedSp + Cost(CommandType.Gather) <= player.sp)
                    items.Add(MakeBubble("채집", new Color(.4f, .9f, .4f), "자원 채집 (SP 2)", () => Queue(CommandType.Gather)));
                if (usedSp + Cost(CommandType.Hunt) <= player.sp)
                    items.Add(MakeBubble("수렵", new Color(.9f, .6f, .3f), "식량 사냥 (SP 2)", () => Queue(CommandType.Hunt)));
                if (usedSp + Cost(CommandType.Attack) <= player.sp)
                    items.Add(MakeBubble("공격", new Color(1f, .35f, .35f), "근접 공격 (SP 2)", () => Queue(CommandType.Attack)));
                if (usedSp + Cost(CommandType.Trade) <= player.sp)
                    items.Add(MakeBubble("거래", new Color(.9f, .8f, .3f), "식량→화폐 (SP 2)", () => Queue(CommandType.Trade)));
                if (usedSp + Cost(CommandType.Hire) <= player.sp)
                    items.Add(MakeBubble("고용", new Color(.7f, .5f, .9f), "화폐 3 → 용병 (SP 2)", () => Queue(CommandType.Hire)));
                if (usedSp + Cost(CommandType.Build) <= player.sp)
                    items.Add(MakeBubble("건설", new Color(.6f, .8f, .5f), "건물 건설 (SP 3)", () => Queue(CommandType.Build)));
                if (usedSp + Cost(CommandType.Upgrade) <= player.sp)
                    items.Add(MakeBubble("강화", new Color(.8f, .7f, .4f), "건물 강화 (SP 3)", () => Queue(CommandType.Upgrade)));
                if (usedSp + Cost(CommandType.Persuade) <= player.sp)
                    items.Add(MakeBubble("설득", new Color(.9f, .5f, .8f), "관계 개선 (SP 2)", () => Queue(CommandType.Persuade)));
            }
            else
            {
        ...
        private void ShowBuildingBubbles(BuildingState building)
        {
            ...
            {
                items.Add(MakeBubble("강화", new Color(.8f, .7f, .4f), "건물 레벨 +1 (SP 3)", () => Queue(CommandType.Upgrade)));
            }
            else
            {
        ...
        private void Queue(CommandType command)
        {
            ...
            if (selectedUnitId >= 0)
            {
                var sel = game.entities.FirstOrDefault(u => u.id == selectedUnitId);
                if (sel != null) ShowUnitBubbles(sel);
            }
        }
        ...
        private void RunDynamic(DynamicActionV1 action)
        {
            ...
            {
                if (effect.type == EffectType.Resource) player.resources.Add(effect.resource, effect.amount);
                else if (effect.type == EffectType.Sp) player.sp = Math.Min(player.maxSp, player.sp + effect.amount);
                else if (effect.type == EffectType.Relation) foreach (var f in game.factions.Where(f => f.id != 1)) f.relationToPlayer = Math.Max(-100, Math.Min(100, f.relationToPlayer + effect.amount));
            }
            action.availableTurn = game.turn + Math.Max(1, action.cooldown);
            ledger.Add("AI 행동 실행: " + action.name);
            RefreshHud();
        }
        ...
        private IEnumerator ResolveTurn()
        {
            ...
            commands.Clear();
            if (!GameRules.HeadquartersAlive(game) && !game.entities.Any(u => u.factionId == 1 && u.alive))
            {
                ledger.Add("본부와 복구 가능한 아군이 모두 사라져 원정이 끝났습니다.");
                Save();
        ...
        private IEnumerator RequestRules()
        {
            ...
            var request = new UnityWebRequest(apiBase.TrimEnd('/') + "/v1/rules/generate", "POST");
            request.uploadHandler = new UploadHandlerRaw(System.Text.Encoding.UTF8.GetBytes(JsonUtility.ToJson(game)));
            ...
            var set = JsonUtility.FromJson<RuleSetV1>(request.downloadHandler.text);
        ...
        {
            if (hudResources == null || game == null) return;
            var player = game.factions.First(f => f.id == 1);
        ...
        private GameSnapshotV1 ReadSave(string key)
        {
            var raw = PlayerPrefs.GetString(key, "");
            if (string.IsNullOrEmpty(raw)) return null;
            var envelope = JsonUtility.FromJson<SaveEnvelope>(raw);
            return envelope != null && envelope.schemaVersion == 1 && envelope.checksum == Hash(envelope.payload) ? JsonUtility.FromJson<GameSnapshotV1>(envelope.payload) : null;
        }

        private void Save()
        {
            var payload = JsonUtility.ToJson(game);
            var envelope = JsonUtility.ToJson(new SaveEnvelope { payload = payload, checksum = Hash(payload) });
    ...
    public sealed class BuildingClickHandler : MonoBehaviour
    {
        ...
        private void OnMouseDown()
        {
            controller?.SelectBuilding(buildingId);
        }
    }
}